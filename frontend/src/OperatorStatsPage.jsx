      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginBottom: 2 }}>{d[valueKey] > 0 ? d[valueKey] : ''}</div>
            <div style={{ width: '100%', height: h, background: color, borderRadius: '3px 3px 0 0', minWidth: 8, transition: 'height 0.3s' }}
              title={`${d[labelKey]}: ${d[valueKey]}`} />
            <div style={{ fontSize: 9, color: 'var(--text-muted)', marginTop: 2, whiteSpace: 'nowrap' }}>{d[labelKey]}</div>
          </div>
        );
      })}
    </div>
...
// KPI Card
function KpiCard({ icon, label, value, sub, color = 'var(--text-primary)' }) {
  return (
    <div style={{
      background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 18px',
      flex: '1 1 180px', minWidth: 160,
    }}>
      <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4 }}>{icon} {label}</div>
...
export default function OperatorStatsPage({ user, onBack }) {
  ...
  if (loading) return (
    <div style={{ padding: 40, textAlign: 'center' }}>
      ...
      <div style={{ marginTop: 12, color: 'var(--text-secondary)' }}>Загрузка статистики...</div>
    </div>
  );
  ...
  if (error) return (
    <div style={{ padding: 40, textAlign: 'center' }}>
      <div style={{ fontSize: 48 }}></div>
      <div style={{ marginTop: 12, color: 'var(--danger)' }}>Ошибка: {error}</div>
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '12px 20px' }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
        ...
        <KpiCard icon="" label="Контактность" value={`${stats.contactRate}%`}
          ...
          color={stats.contactRate >= 50 ? '#16a34a' : stats.contactRate >= 30 ? '#d97706' : 'var(--danger)'} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16, marginBottom: 20 }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: 16 }}>
          ...
          {data.daily.length > 0 && (
            <div style={{ display: 'flex', gap: 16, marginTop: 12, fontSize: 12, color: 'var(--text-secondary)', justifyContent: 'center' }}>
            ...
            <div style={{ maxHeight: 200, overflowY: 'auto' }}>
              {data.topPromises.map((p, i) => (
                <div key={p.id} style={{
                  ...
                  padding: '6px 0', borderBottom: i < data.topPromises.length - 1 ? '1px solid var(--bg-elevated)' : 'none',
                  fontSize: 13,
                }}>
                  <div>
      ...
      {/* All-time summary */}
      <div style={{
        marginTop: 20, background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 10,
        padding: '12px 20px', display: 'flex', gap: 32, fontSize: 13, color: 'var(--text-secondary)',
      }}>
