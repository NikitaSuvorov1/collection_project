      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ fontSize: 10, color: '#6b7280', marginBottom: 2 }}>{d[valueKey] > 0 ? d[valueKey] : ''}</div>
            <div style={{ width: '100%', height: h, background: color, borderRadius: '3px 3px 0 0', minWidth: 8, transition: 'height 0.3s' }}
              title={`${d[labelKey]}: ${d[valueKey]}`} />
            <div style={{ fontSize: 9, color: '#9ca3af', marginTop: 2, whiteSpace: 'nowrap' }}>{d[labelKey]}</div>
          </div>
        );
      })}
    </div>
...
// KPI Card
function KpiCard({ icon, label, value, sub, color = '#111' }) {
  return (
    <div style={{
...
export default function OperatorStatsPage({ user, onBack }) {
  ...
  if (loading) return (
    <div style={{ padding: 40, textAlign: 'center' }}>
      ...
      <div style={{ marginTop: 12, color: '#6b7280' }}>Загрузка статистики...</div>
    </div>
  );
  ...
  if (error) return (
    <div style={{ padding: 40, textAlign: 'center' }}>
      ...
      <div style={{ marginTop: 12, color: '#dc2626' }}>Ошибка: {error}</div>
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '12px 20px' }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 20 }}>
        ...
        <KpiCard icon="📡" label="Контактность" value={`${stats.contactRate}%`}
          ...
          color={stats.contactRate >= 50 ? '#16a34a' : stats.contactRate >= 30 ? '#d97706' : '#dc2626'} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16, marginBottom: 20 }}>
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: 16 }}>
          ...
          {data.daily.length > 0 && (
            <div style={{ display: 'flex', gap: 16, marginTop: 12, fontSize: 12, color: '#6b7280', justifyContent: 'center' }}>
            ...
            <div style={{ maxHeight: 200, overflowY: 'auto' }}>
              {data.topPromises.map((p, i) => (
                <div key={p.id} style={{
                  ...
                  padding: '6px 0', borderBottom: i < data.topPromises.length - 1 ? '1px solid #f3f4f6' : 'none',
                  fontSize: 13,
                }}>
                  <div>
      ...
      {/* All-time summary */}
      <div style={{
        marginTop: 20, background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10,
        padding: '12px 20px', display: 'flex', gap: 32, fontSize: 13, color: '#64748b',
      }}>
