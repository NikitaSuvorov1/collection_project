...
14. [**Комплаенс 230-ФЗ (полный, 11 статей)**](#14-комплаенс-230-фз)
...
|------|-----|----------|------------------|
| Администратор | admin | Системный администратор | Настройка, управление пользователями, административный план взыскания |
...
Интерфейс приложения оформлен в светлой рабочей теме. На каждой странице отображается единая плашка с названием `ИС "Система ДРПЗ"`. Административные функции вынесены в отдельные экраны и показываются только пользователям с соответствующей ролью.

Актуальные скриншоты для документации:

| Экран | Файл |
|-------|------|
| План взыскания на текущий день | `screenshots/plan-vzyskaniya-current-day.png` |
| Распределение клиентов для обзвона | `screenshots/raspredelenie-klientov-obzvon.png` |
...
│       ├── AdminCollectionPlanPage.jsx # План взыскания на текущий день (только admin)
...
│
├── docs/                       # Документы и материалы для пояснительной записки
│   ├── database_chapter.md     # Актуализированная глава по проектированию БД
│   └── screenshots/            # Скриншоты интерфейса для документации
│
├── screenshots/                # Сгенерированные скриншоты страниц приложения
│   ├── plan-vzyskaniya-current-day.png
│   └── raspredelenie-klientov-obzvon.png
│
...
**Административный план взыскания на текущий день:**

На базе эндпоинта `GET /api/assignments/` реализована отдельная frontend-страница `AdminCollectionPlanPage.jsx`. Экран предназначен для строгого контроля ежедневного плана взыскания и доступен только пользователям с ролью `admin`. Пункт меню `План взыскания` отображается только администратору; при прямом переходе пользователя с другой ролью показывается сообщение `Доступ ограничен`.

Страница формирует сводку по текущим назначениям: количество дел к обработке, число сотрудников в плане, количество высокоприоритетных назначений, общую сумму просрочки и средний срок DPD. Основной реестр сортируется по приоритету и сумме просрочки, отображает клиента, маскированный телефон, кредит, сумму просрочки, DPD, ответственного сотрудника и признак административного контроля.
...
| admin | Полный доступ, включая административный план взыскания на текущий день |

Страница `План взыскания` относится к административному контуру управления. На уровне интерфейса она скрыта для ролей `operator`, `senior_operator`, `manager`, `legal_specialist`, `analyst` и `auditor`; компонент дополнительно проверяет `user.role === 'admin'` и при несоответствии показывает экран отказа в доступе. Учётная запись быстрого входа `admin/admin` создаёт frontend-сессию с ролью `admin`.
...
Система реализует **полную проверку** соответствия требованиям **Федерального закона № 230-ФЗ** «О защите прав и законных интересов физических лиц при осуществлении деятельности по возврату просроченной задолженности».

**Файл:** `backend/collection_app/services/compliance_230fz.py`

Комплаенс-модуль обеспечивает:
- **Превентивную проверку** — до каждого контакта (`can_contact`)
- **Пост-валидацию** — после создания воздействия (`validate_intervention`)
- **Автоматическую блокировку** — запрещённые контакты не создаются
- **Журнал нарушений** — двойная запись (ViolationLog + AuditLog)
- **Дашборд комплаенса** — сводная статистика для руководителя

### 14.2 Покрытие 11 статей закона

| # | Статья | Название | Реализация |
|---|--------|----------|------------|
| 1 | Ст. 1 | Ограничение времени контакта | Будни: 08:00–22:00, выходные: 09:00–20:00 |
| 2 | Ст. 2 | Частота контактов | Звонки: макс. 1/день, 2/неделю, 8/месяц; SMS: 2/день, 4/неделю, 16/мес |
| 3 | Ст. 3 | Отказ от взаимодействия | Полный отказ (`contact_refused`) или по каналам (`refused_channels` JSON) |
| 4 | Ст. 4 | Третьи лица | Контакт только при наличии `third_party_consent = True` |
| 5 | Ст. 5 | Запрет угроз | Контроль использования утверждённых скриптов (`approved_script_used`) |
| 6 | Ст. 6 | Идентификация оператора | Оператор обязан представиться (`operator_identified`) |
| 7 | Ст. 7 | Банкротство | Полный запрет контакта при `is_bankrupt = True` |
| 8 | Ст. 8 | Скрытые номера | Запрет звонков со скрытым номером (`caller_number` обязателен) |
| 9 | Ст. 9 | Минимальный интервал | Не менее 4 часов между телефонными звонками |
| 10 | Ст. 10 | Хранение истории | Все контакты логируются в Intervention + AuditLog |
| 11 | Ст. 11 | Персональные данные | Соответствие ФЗ-152, `personal_data_accessed` в AuditLog |

### 14.3 Поля модели Client для 230-ФЗ

```python
# 230-ФЗ: Контроль согласия на контакт
contact_refused = models.BooleanField('Отказ от взаимодействия (ст.8)', default=False)
contact_refused_date = models.DateField('Дата отказа', null=True)
refused_channels = models.JSONField('Каналы отказа', default=list)  # ["phone","sms"]
is_bankrupt = models.BooleanField('Признан банкротом', default=False)
bankruptcy_date = models.DateField('Дата банкротства', null=True)
third_party_consent = models.BooleanField('Согласие на контакт с третьими лицами', default=False)
third_party_consent_date = models.DateField('Дата согласия', null=True)
```

### 14.4 Поля модели Intervention для 230-ФЗ

```python
# 230-ФЗ: Дополнительные требования
caller_number = models.CharField('Исходящий номер (ст.9 — запрет скрытых)')
...
### 14.5 Алгоритм проверки (`can_contact`)
...
└──────┬───────┘
       ...
       v
┌──────────────────────┐      ┌──────────────┐
│ Ст.7: is_bankrupt?   │─ Да ─│  БЛОКИРОВКА  │
└──────┬───────────────┘      └──────────────┘
       │ Нет
       v
┌──────────────────────┐      ┌──────────────┐
│ Ст.3: contact_refused│─ Да ─│  БЛОКИРОВКА  │
└──────┬───────────────┘      └──────────────┘
       │ Нет
       v
┌──────────────────────┐      ┌──────────────┐
│ Ст.3: канал в        │─ Да ─│  БЛОКИРОВКА  │
│  refused_channels?   │      └──────────────┘
└──────┬───────────────┘
       │ Нет
       v
┌──────────────────────┐      ┌──────────────┐
│ Ст.1: допустимое     │─ Нет─│  БЛОКИРОВКА  │
│  время?              │      └──────────────┘
└──────┬───────────────┘
       │ Да
       v
┌──────────────────────┐      ┌──────────────┐
│ Ст.2: лимит частоты  │─ Нет─│  БЛОКИРОВКА  │
│  не превышен?        │      └──────────────┘
└──────┬───────────────┘
       │ Да
       v
┌──────────────────────┐      ┌──────────────┐
│ Ст.4: если 3rd party │─ Нет─│  БЛОКИРОВКА  │
│  есть ли consent?    │      └──────────────┘
└──────┬───────────────┘
       │ Да
       v
...
### 14.7 Автоматическая проверка при создании Intervention

При создании воздействия через API (`InterventionViewSet.perform_create`) система:

1. Вызывает `compliance_230fz.can_contact()` для проверки допустимости
2. Если контакт **заблокирован** — возвращает HTTP 403 с описанием нарушений
3. Если контакт **разрешён** — создаёт Intervention и записывает в AuditLog
4. Исключает клиентов с `is_bankrupt=True` из очереди Assignment

### 14.8 API комплаенса

| Метод | URL | Описание |
|-------|-----|----------|
| GET | `/api/compliance/check/?client_id=123&contact_type=phone` | Проверка допустимости контакта |
| GET/POST | `/api/compliance/bankruptcy/?client_id=123` | Проверка/регистрация банкротства |
| GET | `/api/compliance/summary/` | Сводка: заблокировано, проверки, рейт |
| GET | `/api/violations/?client_id=&operator_id=&rule_type=&severity=` | Журнал нарушений с фильтрами |

---

## 15. Roadmap развития

### 15.1 Текущий статус (v4.1)

| Компонент | Статус | Описание |
|-----------|--------|----------|
| Архитектура | ✅ MVP+ | Services layer, RBAC, Audit |
| ML модели | ✅ Полные | RF (просрочка, 26 признаков), GB (одобрение, 31 признак), Pipeline |
| Security | ✅ Реализована | Rate limiting, Audit, Security headers, Request validation |
| Collection процессы | ✅ Полный цикл | Pre → Soft → Hard → Legal + реструктуризация |
| Workflow Engine | ✅ Реализован | Rules Engine с автопереходами, scheduled actions |
| Данные | ✅ Качество | ~21k CreditState, ~19k Payments, ставки по продуктам |
| Рабочий стол оператора | ✅ Полный | Персональная очередь, call workflow, таймер, NBA |
| Client 360° | ✅ Реализован | Реальные API, NBA, история воздействий |
| Комплаенс 230-ФЗ | ✅ Полный (11 статей) | ViolationLog, can_contact, банкротство, частота, время |
| Дашборд аналитики | ✅ Реализован | Статистика операторов, daily/hourly, PTP, каналы |
| API документация | ✅ Swagger / ReDoc | drf-spectacular, OpenAPI 3.0, 30+ endpoints |
| Психотипирование | ✅ Реализовано | 5 психотипов, ClientBehaviorProfile, NBA |
| Smart-скрипты | ✅ Реализованы | Самообучающиеся, по психотипам, процент успеха |
| Скоринг дашборд | ✅ Реализован | Грейды A-E, баллы 300-850, expected profit |
| A/B тестирование | ✅ Реализовано | Группа A (случайное) vs B (умное) распределение |
...
| Административный план взыскания | ✅ Реализован | Строгий реестр назначений на текущий день, доступ только для роли admin |
...
| 4.0.0 | 2026-03-11 | **Мажорное обновление документации.** Полная карта 30+ моделей данных (CollectionCase, ClientBehaviorProfile, ViolationLog, ScoringResult, CreditApplication, MLModelVersion и др.). 24 раздела API (30+ endpoints): поведенческие профили, NBA, smart-скрипты, compliance-алерты, прогнозы возврата, дашборд/аналитика, прогнозирование просрочки, обучение ML, скоринг, A/B тесты, аудит, ежедневные состояния кредитов. Полный комплаенс 230-ФЗ (11 статей) с алгоритмом can_contact, полями Client/Intervention, блок-схемой. Обновлена структура проекта (новые страницы frontend, management commands). Обновлён roadmap с текущим статусом v4.0. |
| 4.1.0 | 2026-05-07 | Обновлена frontend-документация: светлая тема приложения, единая плашка `ИС "Система ДРПЗ"` на страницах, административная страница `План взыскания` с доступом только для роли `admin`. Добавлены ссылки на скриншоты `screenshots/plan-vzyskaniya-current-day.png` и `screenshots/raspredelenie-klientov-obzvon.png`. Добавлена актуализированная глава по проектированию БД: `docs/database_chapter.md`. |
